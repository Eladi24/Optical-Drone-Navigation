# Vendored verbatim (architecture/logic unmodified) from
# github.com/fabio-sim/LightGlue-ONNX, tag v0.1.0 (commit 1da3ab3a0ed19accb77804ea0616ff470a2db357),
# path lightglue_onnx/disk.py -- Apache-2.0 (LICENSE confirmed at that tag).
#
# WHY VENDORED INSTEAD OF PIP-INSTALLED: the repo's own setup.py packages only its SEPARATE
# `lightglue/` directory (a vendored copy of the original cvg/LightGlue-style dict-input API,
# NOT this ONNX-exportable tensor-input API) under the installable name `lightglue` -- confirmed
# directly by `pip install "git+...@v0.1.0"` producing a `DISK.forward(data: dict)` signature
# that fails to export (see scripts/export_disk_lightglue.py's module docstring). This
# `lightglue_onnx/` directory -- the one this file actually comes from -- is never packaged by
# that setup.py at all, so it has to be vendored directly, the same reason export_xfeat.py
# vendors XFeat's architecture (no installable package exposes the exportable API).
