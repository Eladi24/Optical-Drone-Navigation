    ...
    PositionAlgorithm algorithm)
{
    ...
    for (int step = 0; step < max_steps; step++)
    {
        ...
        // Check if we're approaching the end of the path
        const auto &current_actual_pos = drone.getPosition();
        double dist_to_end_lat = (end_lat - current_actual_pos.first) * meters_per_degree_lat;
        double dist_to_end_lng = (end_lng - current_actual_pos.second) * meters_per_degree_lng;
        double dist_to_end = std::sqrt(dist_to_end_lat * dist_to_end_lat + 
                                      dist_to_end_lng * dist_to_end_lng);
        
        // Outlier detection - check if position jumped too far
        if (!algorithm_positions.empty()) {
            ...
            double max_expected_jump = speed * sim_dt * 3.0;  // 3x safety factor
            ...
            if (jump_distance > max_expected_jump) {
                ...
                // Blend with GPS position to correct the outlier
                estimated_position.first = estimated_position.first * 0.3 + 
                                          current_actual_pos.first * 0.7;
                estimated_position.second = estimated_position.second * 0.3 + 
                                           current_actual_pos.second * 0.7;
                match_confidence *= 0.4;
            }
        }
        
        // Near destination - blend more heavily with GPS to avoid final spike
        if (dist_to_end < 20.0) {
            double gps_weight = std::min(1.0, (20.0 - dist_to_end) / 20.0);  // 0 to 1 as we approach
            estimated_position.first = estimated_position.first * (1 - gps_weight) + 
                                      current_actual_pos.first * gps_weight;
            estimated_position.second = estimated_position.second * (1 - gps_weight) + 
                                       current_actual_pos.second * gps_weight;
        }
    ...
    // After the simulation loop ends, create and display error graph
    ...
    // Force refresh
    cv::waitKey(1);  // Give UI time to update
    
    // Save the error graph to file
    bool save_success = cv::imwrite("error_graph_" + algorithm_name + ".png", error_graph);
    if (save_success) {
        std::cout << "Error graph saved to error_graph_" + algorithm_name + ".png" << std::endl;
    } else {
    ...
    PositionAlgorithm algorithm)
{
    ...
    for (int step = 0; step < max_steps; step++)
    {
        ...
        // Check if we're approaching the current waypoint
        if (current_waypoint < waypoints.size()) {
            double target_lat = waypoints[current_waypoint].first;
            double target_lng = waypoints[current_waypoint].second;
            ...
            if (!algorithm_positions.empty()) {
                ...
                if (jump_distance > max_expected_jump) {
                    std::cout << "Warning: Outlier detected at step " << step 
                              << " (jumped " << jump_distance << "m)" << std::endl;
                    
                    estimated_position.first = estimated_position.first * 0.3 + 
                                              drone.getPosition().first * 0.7;
                    estimated_position.second = estimated_position.second * 0.3 + 
                                               drone.getPosition().second * 0.7;
                    match_confidence *= 0.4;
                }
            }
            
            // Blend with GPS when very close to waypoint
            if (dist_to_waypoint < 20.0) {
                double gps_weight = std::min(1.0, (20.0 - dist_to_waypoint) / 20.0);
                estimated_position.first = estimated_position.first * (1 - gps_weight) + 
                                          drone.getPosition().first * gps_weight;
                estimated_position.second = estimated_position.second * (1 - gps_weight) + 
                                           drone.getPosition().second * gps_weight;
                
                if (gps_weight > 0.5) {
                    std::cout << "Near waypoint " << current_waypoint 
                              << " (" << dist_to_waypoint << "m) - GPS weight: " 
                              << gps_weight << std::endl;
                }
            }
        }
    ...
    // Save the error graph to file
    bool save_success = cv::imwrite("error_graph_" + algorithm_name + "_waypoints.png", error_graph);
    if (save_success) {
        std::cout << "Error graph saved to error_graph_" + algorithm_name + "_waypoints.png" << std::endl;
    } else {
        std::cout << "Failed to save error graph!" << std::endl;
    }
...
// Function to create error graph visualization
